...
export function DevinChat(props: {
					...
					if (s.status_detail && isWaitingForUser(s.status_detail)) {
						...
						if (appStore.devinSettings.notifyWaiting) {
							showLocalNotification(
								"Devin needs input",
								s.title || props.session.session_id,
							);
						}
					} else {
						setWaiting(null);
	...
	async function send(text: string, attachmentUrls: string[] = []) {
		...
		try {
			...
			setWaiting(null);
			const { messages: list } = await listDevinMessages(
				session().session_id,
				100,
			);
			setMessages(list);
			...
			<div class="mb-3 flex items-center justify-between">
				<div class="flex items-center gap-2 overflow-hidden">
					...
					<div class="min-w-0">
						...
						<p
							class={`text-xs ${session().status === "error" ? "text-danger" : "text-text-secondary"}`}
						>
							{statusLabel()}
						</p>
					</div>
