...
export function SettingsModal(props: { onClose: () => void }) {
	...
	const [devinProxy, setDevinProxy] = createSignal(
		appStore.devinSettings.useProxy,
	);
	const [devinNotifyDone, setDevinNotifyDone] = createSignal(
		appStore.devinSettings.notifyCompleted,
	);
	const [devinNotifyWait, setDevinNotifyWait] = createSignal(
		appStore.devinSettings.notifyWaiting,
	);
	...
	function setStartup(tabId: string) {
		setGlobalSetting("startup", tabId);
		showStatus(
			`Startup tab set to ${appStore.tabs.find((t) => t.id === tabId)?.label ?? tabId}`,
			"success",
		);
	}
	...
	function setClockDefault(tab: string) {
		setClockSubTab(tab);
		setGlobalSetting("defaultClockSubTab", tab);
	}
